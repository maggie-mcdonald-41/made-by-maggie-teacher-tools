...
(function () {
  ...
  const btnCollapse = document.getElementById("rp-tts-collapse");
  const ttsBody = document.getElementById("rp-tts-body");

  // If the widget isn't present, do nothing.
if (!floatRoot || !voiceSelect || !speedSelect || !btnReadPassage || !btnReadQuestion || !btnToggle || !btnStop || !btnCollapse || !ttsBody) {
  return;
}
  ...
  const TTS_COLLAPSED_STORAGE_KEY = "rp7_ttsCollapsed";

  function getInitialCollapsedState() {
    try {
      return window.localStorage.getItem(TTS_COLLAPSED_STORAGE_KEY) === "true";
    } catch (_) {
      return false;
    }
  }

  function applyCollapsedState(isCollapsed, shouldSave = true) {
    floatRoot.classList.toggle("is-collapsed", isCollapsed);
    ttsBody.hidden = isCollapsed;

    btnCollapse.textContent = isCollapsed ? "▴" : "−";
    btnCollapse.setAttribute("aria-expanded", String(!isCollapsed));
    btnCollapse.setAttribute(
      "aria-label",
      isCollapsed ? "Expand read aloud controls" : "Collapse read aloud controls"
    );
    btnCollapse.title = isCollapsed ? "Expand read aloud controls" : "Collapse read aloud controls";

    if (shouldSave) {
      try {
        window.localStorage.setItem(TTS_COLLAPSED_STORAGE_KEY, String(isCollapsed));
      } catch (_) {}
    }
  }
...
function getSelectedRate() {
  ...
  // ---- UI events ----
  applyCollapsedState(getInitialCollapsedState(), false);

  btnCollapse.addEventListener("click", () => {
    const shouldCollapse = !floatRoot.classList.contains("is-collapsed");
    applyCollapsedState(shouldCollapse, true);
  });
