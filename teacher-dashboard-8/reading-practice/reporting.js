...
function buildSummary() {
  ...
  function setSessionInfo(info = {}) {
    ...
    state.sessionInfo = {
      ...
      ownerEmail,
      gradeLevel: cleanString(info.gradeLevel || info.grade || window.READING_GRADE_LEVEL || "8"),
      grade: cleanString(info.grade || info.gradeLevel || window.READING_GRADE_LEVEL || "8"),      
  ...
  function recordQuestionResult(q, payload = {}) {
    ...
    const entry = {
      ...
      standards: Array.isArray(q.standards) ? q.standards.slice() : [],
      isCorrect,
      raw: payload
    };
