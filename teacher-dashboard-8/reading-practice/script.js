...
function logQuestionResult(q, extra = {}) {
  ...
  const base = {
    ...
    standards: Array.isArray(questionObj.standards) ? questionObj.standards.slice() : []
  };
...
function beginTrainerSession({ studentName, sessionCode }) {
  ...
  if (!ownerEmail) {
    try {
      ...
      ownerEmail =
        window.localStorage.getItem("rp8_lastOwnerEmail") || "";
    } catch (e) {
      // ignore
    }
  }
        ...
        if (
          ...
          if (!resume) {
            ...
            try {
              ...
              const localKey = `rp_g${READING_GRADE_LEVEL}_progress_${sessionCode}_${studentKey}`;
...
function equalizeChoiceButtonHeights(container = questionOptionsEl) {
  if (!container) return;

  const groups = container.querySelectorAll(".choice-list");
  groups.forEach((group) => {
    ...
    const maxHeight = Math.max(...buttons.map((btn) => btn.scrollHeight));
    buttons.forEach((btn) => {
...
// ====== MEDIA RENDERING (audio / video with reliable subtitles support) ======
const mediaCaptionCache = new Map();

function parseVttTime(timeString) {
  const clean = String(timeString || "").trim().replace(",", ".");
...
function parseVttCues(vttText) {
  ...
  blocks.forEach((block) => {
    ...
    if (lines[0].toUpperCase() === "WEBVTT") return;
...
async function loadVttCues(captionsSrc) {
  if (!captionsSrc) return [];
  if (mediaCaptionCache.has(captionsSrc)) return mediaCaptionCache.get(captionsSrc);
  ...
  try {
    ...
    mediaCaptionCache.set(captionsSrc, cues);
    return cues;
  } catch (error) {
    console.warn("[Media captions] Failed to load VTT:", captionsSrc, error);
    return [];
  }
}
...
function renderMediaIfPresent(q) {
  ...
  // Use a true audio player for podcasts.
  // Native browser captions are unreliable for audio-only files, so captions are shown in a custom box below.
  ...
  if (captions) {
    ...
    captionBox.className = "media-live-caption";
    captionBox.setAttribute("aria-live", "polite");
    captionBox.textContent = "Captions will appear here when the podcast plays.";
    wrapper.appendChild(captionBox);

    loadVttCues(captions).then((cues) => {
      if (!captionBox) return;

      if (!cues.length) {
        captionBox.textContent = "Captions could not be loaded.";
        captionBox.classList.add("caption-muted");
        return;
      }
      ...
      function updateCaptionText() {
        ...
        // Pick the latest cue whose start time has passed.
        // This is more reliable for audio captions than exact start/end matching.
        let activeCue = null;
        ...
        for (let i = 0; i < cues.length; i++) {
          ...
          const nextStart = nextCue ? nextCue.start : Number.POSITIVE_INFINITY;

          if (currentTime >= cueStart && currentTime < nextStart) {
            activeCue = cue;
            break;
          }
        }

        if (activeCue && !player.ended) {
          captionBox.textContent = activeCue.text;
          captionBox.classList.remove("caption-muted");
          return;
        }

        if (player.ended) {
          captionBox.textContent = "Podcast complete.";
          captionBox.classList.add("caption-muted");
          return;
        }
      ...
      player.addEventListener("timeupdate", updateCaptionText);
      player.addEventListener("seeked", updateCaptionText);
      player.addEventListener("play", updateCaptionText);
      player.addEventListener("playing", updateCaptionText);
      player.addEventListener("pause", updateCaptionText);
      player.addEventListener("ended", updateCaptionText);
    });
  }
...
function renderQuestion() {
  ...
  questionTypeLabelEl.textContent = getTypeLabel(q);
...
if (q.stimulus) {
  ...
  // Make answer buttons the same height so students cannot use answer length as a visual clue.
  requestAnimationFrame(() => equalizeChoiceButtonHeights(questionOptionsEl));
