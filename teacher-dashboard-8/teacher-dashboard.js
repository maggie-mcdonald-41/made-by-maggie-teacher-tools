...
const READING_GRADE_LEVEL = "8";
const DASHBOARD_BASE_PATH = "/teacher-dashboard-8";
...
// ---------- HISTORY STORAGE KEY ----------
const HISTORY_KEY = "rp8_teacherSessionHistory_v1";
...
const BENCHMARK_MAP = {
  q4: "benchmarks/mgb_8th_grade_benchmark.json"
};
...
async function startBenchmarkSession() {
  ...
  try {
    ...
    try {
      window.localStorage.setItem("rp8_lastSessionCode", normalizedSession);
      window.localStorage.setItem("rp8_lastSet", "benchmark");
      window.localStorage.setItem("rp8_lastLevel", "benchmark");
...
alert("Benchmark links are ready. Copy the student link or co-teacher link as needed.");
    ...
    if (loadStatusEl) {
      loadStatusEl.textContent =
        "Could not load benchmark JSON. Check benchmarks/mgb_8th_grade_benchmark.json.";
    }
  } finally {
    if (startBenchmarkBtn) {
...
(function wirePracticeLevelSelector() {
  ...
  // restore last choice
  try {
    const last = window.localStorage.getItem("rp8_lastLevel");
...
if (typeof updateCurrentViewSummary === "function") updateCurrentViewSummary();
  ...
  levelSelect.addEventListener("change", () => {
    ...
    try {
      window.localStorage.setItem("rp8_lastLevel", currentLevelParam);
    } catch (e) {}
    refreshOutputs();
  });
})();
...
(function wirePracticeSetSelector() {
  ...
  const persist = () => {
    try {
      ...
      window.localStorage.setItem("rp8_lastSet", normalizeSetParam(setSelect.value));
    } catch (e) {}
  };
  ...
  try {
    const last = normalizeSetParam(window.localStorage.getItem("rp8_lastSet"));
...
function clearDashboardForSignedOut() {
  ...
  try {
    window.localStorage.removeItem("rp8_lastSessionCode");
    window.localStorage.removeItem("rp8_lastSessionLink");
    window.localStorage.removeItem("rp8_lastCoTeacherLink");
    window.localStorage.removeItem("rp8_lastOwnerEmail");
  } catch (e) {}
...
const HISTORY_DELETED_KEY = "rp8_teacherSessionHistoryDeleted_v1";
...
const selectedLevel = isBenchmarkSession ? "benchmark" : currentLevelParam || "on";
  ...
  const selectedOwnerEmail = (() => {
    ...
    if (!owner) {
      try {
        owner = window.localStorage.getItem("rp8_lastOwnerEmail") || "";
      } catch (e) {}
    }
...
if (entry.practiceSet && setSelect) {
  ...
  setSelect.value = v;
  currentSetParam = v;
  try { localStorage.setItem("rp8_lastSet", v); } catch (e) {}
}
...
if (entry.practiceLevel) {
  ...
  currentLevelParam = v;
  try { localStorage.setItem("rp8_lastLevel", v); } catch (e) {}
...
if (isBenchmarkHistory) {
  ...
  try {
    localStorage.setItem("rp8_lastSet", "benchmark");
    localStorage.setItem("rp8_lastLevel", "benchmark");
  } catch (e) {}
} else {
...
function enableMonitorButton(sessionCodeRaw) {
  ...
  monitorSessionBtn.onclick = (() => {
    ...
    if (CURRENT_SESSION_OWNER_OVERRIDE) {
      ...
      try {
        ownerEmail = window.localStorage.getItem("rp8_lastOwnerEmail") || "";
      } catch (e) {}
    }
    ...
    const url = `${window.location.origin}${DASHBOARD_BASE_PATH}/reading-practice/live-monitor.html?${params.toString()}`;
...
function buildStudentLink(sessionCode) {
  ...
  const baseUrl = `${window.location.origin}${DASHBOARD_BASE_PATH}/reading-practice/index.html`;
  const params = new URLSearchParams();
  ...
  try {
    window.localStorage.setItem("rp8_lastSessionCode", cleanSession);
    window.localStorage.setItem("rp8_lastSet", setParam);
    if (level) window.localStorage.setItem("rp8_lastLevel", level);
    if (ownerEmail) window.localStorage.setItem("rp8_lastOwnerEmail", ownerEmail);
  } catch (e) {
    // non-fatal
  }
...
function buildCoTeacherLink(sessionCode) {
  ...
  const baseUrl = `${window.location.origin}${DASHBOARD_BASE_PATH}/teacher-dashboard.html`;
  const params = new URLSearchParams();
  ...
  try {
    window.localStorage.setItem("rp8_lastCoTeacherLink", link);
    window.localStorage.setItem("rp8_lastSet", setParam);
    if (level) window.localStorage.setItem("rp8_lastLevel", level);
  } catch (e) {
    // non-fatal
...
function buildBenchmarkCoTeacherLink(sessionCode, benchmarkKey = "q4") {
  ...
  try {
    window.localStorage.setItem("rp8_lastCoTeacherLink", link);
    window.localStorage.setItem("rp8_lastOwnerEmail", ownerEmail);
    window.localStorage.setItem("rp8_lastSet", "benchmark");
    window.localStorage.setItem("rp8_lastLevel", "benchmark");
  } catch (e) {
    // non-fatal
...
function startNewSession() {
  ...
  try {
    window.localStorage.setItem("rp8_lastSessionCode", normalizedSession);
    window.localStorage.setItem("rp8_lastSessionLink", studentLink);
...
const isBenchmarkUrl =
  ...
  (params.get("level") || "").toLowerCase() === "benchmark";
    ...
    try {
      localStorage.setItem("rp8_lastLevel", currentLevelParam);
    } catch (e) {}
...
if (isBenchmarkUrl) {
  ...
  switchDashboardTab("practice");
    ...
    try {
      localStorage.setItem("rp8_lastSet", urlSet);
    } catch (e) {}
...
(function restoreLastSession() {
  try {
    const lastCode = window.localStorage.getItem("rp8_lastSessionCode");
    const lastLink = window.localStorage.getItem("rp8_lastSessionLink");
    const lastCoLink = window.localStorage.getItem("rp8_lastCoTeacherLink");
    const lastOwner = window.localStorage.getItem("rp8_lastOwnerEmail");
